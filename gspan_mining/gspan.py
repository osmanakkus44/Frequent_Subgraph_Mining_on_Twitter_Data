...
class gSpan(object):
    ...
    def time_stats(self):
        """Print stats of time."""
        func_names = ['_read_graphs', '_generate_1edge_frequent_subgraphs']
    ...
    def run(self):
        ...
        for vevlb, projected in root.items():
            ...
            self._subgraph_mining(projected, nx_graphs)
            #print(nx_graphs)
            self._DFScode.pop()
        
        #print(type(nx_graphs))
        #for x in nx_graphs:
            #print(type(x))
    ...
    def _subgraph_mining(self, projected, nx_graphs):
        self._support = self._get_support(projected)
        if self._support < self._min_support:
            return
        if not self._is_min():
            return
        a_graph = self._report(projected)
        nx_graphs.append(a_graph)
        ...
        for to, elb in backward_root:
            ...
            self._subgraph_mining(backward_root[(to, elb)], nx_graphs)
            self._DFScode.pop()
        # forward
        ...
        for frm, elb, vlb2 in forward_root:
            ...
            self._subgraph_mining(forward_root[(frm, elb, vlb2)], nx_graphs)
            self._DFScode.pop()
